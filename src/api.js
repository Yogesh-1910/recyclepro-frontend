...
export const api = {
  ...
  updateScrap:        (id, body)  => req('PUT',    `/scrap/${id}`, body),
  deleteScrap:        (id)        => req('DELETE', `/scrap/${id}`),
  getProduction:      (date)      => req('GET',    `/production${date ? `?date=${date}` : ''}`),
  getProductionEntry: (id)        => req('GET',    `/production/${id}`),
  addProduction:      (body)      => req('POST',   '/production', body),
  updateProduction:   (id, body)  => req('PUT',    `/production/${id}`, body),
  ...
  updateSale:         (id, body)  => req('PUT',    `/sales/${id}`, body),
  ...
  getOpeningStock:    ()          => req('GET',    '/opening-stock'),
  updateOpeningStock: (type,body) => req('PUT',    `/opening-stock/${type}`, body),
};